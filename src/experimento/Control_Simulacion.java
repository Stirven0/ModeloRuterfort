...
public class Control_Simulacion extends ControlPanelAbstract {
    ...
    public void playEvent(EventHandler<ActionEvent> event) {
        playButton.setOnAction(event);
    }

    public void pauseEvent(EventHandler<ActionEvent> event){
        pauseButton.setOnAction(event);
    }

    public void rebootEvent(EventHandler<ActionEvent> event){
        rebootButton.setOnAction(event);
    }

    public void velocidadEvent(EventHandler<MouseEvent> event){
        velocidaSlider.setOnMouseDragged(event);
    }

    public void intencidadEvent(EventHandler<MouseEvent> event){
        intencidaSlider.setOnMouseDragged(event);
    }
    public double getVelocidad(){
        if (velocidaSlider != null) {
            return velocidaSlider.getValue();
        }else{
            return defaultValue;
        }
    }
    
    public double getIntencidad(){
        if (intencidaSlider != null) {
            return intencidaSlider.getValue();
        } else {
            return defaultValue;
        }
    }
